...
void ApplyANSIColors(NSTextStorage *textStorage, NSRange textStorageRange, NSString *escapeSeq) {
	...
	if ([textStorage.string rangeOfString:escapeSeq options:0 range:textStorageRange].location == NSNotFound) return;
...
*/
	...
	{
		if (firstPass) { } // The first component in the array won't need processing.
		//	If there was an escape sequence at the very beginning of the string, then the first component in the array will be an empty string.
		//	Otherwise the first component is everything before the first escape sequence.

		else { // componentSeqRange : Range of escape sequence within component, e.g. "fg124,12,12;"
			...
			else if (resetFg || resetBg) { // Reset attributes

				resetFg  ? [attrs removeObjectForKey:NSForegroundColorAttributeName]
							: [attrs removeObjectForKey:NSBackgroundColorAttributeName];
				// Mark the range of the sequence (escape sequence + reset color sequence).
				NSRange seqRange = (NSRange){
					.location = componentRange.location - [escapeSeq length],
			...
			else if (fg || bg)
			{
				...
				if (range_separator.location != NSNotFound) {	// Extract red substring
					...
					if (range_separator.location != NSNotFound) { // Extract green substring
						...
						if (range_separator.location != NSNotFound) { // Extract blue substring
				...
				if (str_r && str_g && str_b) { // Parse rgb values and create color
					...
					color = [NSColor colorWithCalibratedRed:(r/255.0)
																 green:(g/255.0)
																  blue:(b/255.0)
																 alpha:1.0];
					fg ? [attrs setObject:color forKey:NSForegroundColorAttributeName]
				...
				else	{ // Wasn't able to parse a color code
	...
	if ([seqRanges count])
		for (NSValue *seqRangeValue in seqRanges)
			[textStorage addAttributes:@{	NSFontAttributeName 					: [NSFont systemFontOfSize:0.001],
	 NSForegroundColorAttributeName	: NSColor.clearColor } range:[seqRangeValue rangeValue]];
}
- (void)fixAttributesInRange:(NSRange)aRange
{
	// This method "overrides" the method within NSTextStorage.
	// First we invoke the actual NSTextStorage method. This allows it to do any normal processing.
	...
	if (xcode_colors && (strcmp(xcode_colors, "YES") == 0))	ApplyANSIColors(self, aRange, XCODE_COLORS_ESCAPE);
}
...
{
	if (!sourceSel || !sourceClass || !destinationClass) return NSLog(@"XcodeColors: Missing parameter to ReplaceInstanceMethod"), nil;

	destinationSel = destinationSel ?: sourceSel;

	Method sourceMethod = class_getInstanceMethod(sourceClass, sourceSel);
	if (!sourceMethod) return NSLog(@"XcodeColors: Unable to get sourceMethod"), nil;
	...
	if (!destinationMethod) return NSLog(@"XcodeColors: Unable to get destinationMethod"), nil;

	IMP newImplementation = method_getImplementation(destinationMethod);
	if (!newImplementation) return  NSLog(@"XcodeColors: Unable to get newImplementation"), nil;
...
+ (void) pluginDidLoad: (NSBundle*)plugin {
	...
	char *xcode_colors = getenv(XCODE_COLORS);
	if (xcode_colors && (strcmp(xcode_colors, "YES") != 0))	return;
